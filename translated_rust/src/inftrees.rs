...
pub fn inflate_table(
    ...
    let base: &[::core::ffi::c_ushort];
    let extra: &[::core::ffi::c_ushort];
    ...
    root = *bits;
    if root > MAXBITS as ::core::ffi::c_uint {
        return Err(1);
    }
    ...
    while (sym as usize) < codes {
        let length = lens[sym as usize] as usize;
        if length != 0 {
            let offset = offs[length] as usize;
            offs[length] = offs[length].wrapping_add(1);
            work[offset] = sym as ::core::ffi::c_ushort;
        }
        sym = sym.wrapping_add(1);
    }
    ...
    loop {
        ...
        if (symbol as ::core::ffi::c_uint).wrapping_add(1) < match_0 {
            ...
            let index = symbol - match_0 as usize;
            let Some(&op) = extra.get(index) else {
                return Err(1);
            };
            let Some(&value) = base.get(index) else {
                return Err(1);
            };
            here.op = op as ::core::ffi::c_uchar;
            here.val = value;
        } else {
        ...
        if len > root && huff & mask != low {
            ...
            low = huff & mask;
            let Some(slot) = table.get_mut(low as usize) else {
                return Err(1);
            };
            slot.op = curr as ::core::ffi::c_uchar;
            slot.bits = root as ::core::ffi::c_uchar;
            slot.val = next as ::core::ffi::c_ushort;
        }
    }
    if huff != 0 as ::core::ffi::c_uint {
        ...
        here.bits = len.wrapping_sub(drop_0) as ::core::ffi::c_uchar;
        here.val = 0 as ::core::ffi::c_int as ::core::ffi::c_ushort;
        let Some(slot) = table.get_mut(next + huff as usize) else {
            return Err(1);
        };
        *slot = here;
    }
