...
impl<'a> PendingStorageView<'a> {
    ...
    /// Return a checked immutable pending-output range.  This keeps pending
    /// draining on the same bounded callback-storage view that performed the
    /// preceding bit flush, instead of handing a private helper an unbounded
    /// bare slice.
    fn pending_output_range(&self, offset: usize, len: usize) -> Option<&[crate::stdlib::Bytef]> {
...
pub unsafe extern "C" fn deflate_ffi(
    ...
    // This immutable view is only needed by the later tree-flush paths.
    // Establish it after the caller-buffer preflight has accepted the call.
    let pending_buffer = core::slice::from_raw_parts(
        (*s).pending_buf
        ...
        || flush != crate::zlib_h::Z_NO_FLUSH && (*s).status != crate::src::deflate::FINISH_STATE
    {
        ...
        bstate = (if (*s).level == 0 as ::core::ffi::c_int {
            crate::deflate_stored_at_ffi_boundary!(s, flush) as ::core::ffi::c_uint
        } else if (*s).strategy == crate::zlib_h::Z_HUFFMAN_ONLY {
            crate::deflate_huff_at_ffi_boundary!(s, flush) as ::core::ffi::c_uint
            ...
            (match configuration_table[(*s).level as usize].function {
                DeflateCompressionFunction::Stored => {
                    crate::deflate_stored_at_ffi_boundary!(s, flush)
                }
                DeflateCompressionFunction::Fast => {
                    'fast: {
                        // As with slow, establish each callback-owned allocation
                        // once at the exported boundary.  The safe core retains
                        // those views while caller input and output are borrowed
                        // only in their separate, sequential scopes below.
                        let state = &mut *s;
                        if state.window.is_null() || state.head.is_null() || state.prev.is_null() {
                            break 'fast need_more;
                        }
                        let pending = core::slice::from_raw_parts_mut(
                            state
                                .pending_buf
                        ...
                        let Some(mut working) =
                            DeflateWorkingSet::new(state, pending, window, head, prev)
                        else {
                            break 'fast need_more;
                        };
                        ...
                        loop {
                            ...
                            {
                                let (next_in, avail_in, total_in, adler) = {
                                    let stream = &mut *state.strm;
                                    (
                                        stream.next_in,
                                        stream.avail_in,
                                ...
                                let input = if avail_in == 0 {
                                    ...
                                    core::slice::from_raw_parts(next_in, avail_in as usize)
                                };
                                let progress =
                                    working.refill(state, input, avail_in, total_in, adler);
                                let stream = &mut *state.strm;
                                stream.avail_in = progress.avail_in;
                                stream.total_in = progress.total_in;
                                stream.adler = progress.adler;
                                ...
                                match deflate_match_refill_action(state.lookahead, flush) {
                                    DeflateMatchRefillAction::Continue => {}
                                    DeflateMatchRefillAction::NeedMore => break 'fast need_more,
                                    DeflateMatchRefillAction::EndBlock => break,
                                }
                            }

                            let flush_block = match deflate_fast_step_core(state, &mut working) {
                                DeflateFastStep::NeedMore => break 'fast need_more,
                                DeflateFastStep::Continue { flush_block } => flush_block,
                            };
                            if !flush_block {
                                continue;
                            }
                            ...
                            let (next_out, avail_out, total_out) = {
                                let stream = &mut *state.strm;
                            ...
                            let output =
                                core::slice::from_raw_parts_mut(next_out, avail_out as usize);
                            let Some(result) =
                                working.flush_pending_into(state, output, avail_out, total_out)
                            else {
                                break 'fast need_more;
                            };
                            let stream = &mut *state.strm;
                        ...
                        state.insert = deflate_insert_after_block(state.strstart);
                        let finish = flush == crate::zlib_h::Z_FINISH;
                        if !finish && state.sym_next == 0 {
                            break 'fast block_done;
                        }
                        let stored_len = deflate_block_len(state.strstart, state.block_start);
                        let Some((storage, stored_data)) =
                            working.pending_and_block_data(state.block_start, stored_len)
                        else {
                            break 'fast if finish { finish_started } else { need_more };
                        };
                        {
                            let stream = &mut *state.strm;
                            crate::src::trees::tr_flush_block_core(
                        ...
                        let output = core::slice::from_raw_parts_mut(next_out, avail_out as usize);
                        let Some(result) =
                            working.flush_pending_into(state, output, avail_out, total_out)
                        else {
                            break 'fast if finish { finish_started } else { need_more };
                        ...
                        if finish {
                            ...
                            {
                                break 'fast block_state;
                            }
                            break 'fast finish_done;
                        }
                        if stream.avail_out == 0 {
                            break 'fast need_more;
                        }
                ...
                DeflateCompressionFunction::Slow => {
                    'slow: {
                        // The exported boundary establishes the four callback-owned
                        // allocations once for this invocation.  The safe strategy
                        // core retains those views; caller input and output views are
                        // created in separate, non-overlapping scopes below.
                        let state = &mut *s;
                        if state.window.is_null() || state.head.is_null() || state.prev.is_null() {
                        ...
                        loop {
                            ...
                            let flush_block = match deflate_slow_step_core(state, &mut working) {
                                DeflateSlowStep::NeedMore => break 'slow need_more,
                                DeflateSlowStep::Continue { flush_block } => flush_block,
                            };
                            if !flush_block {
                                continue;
                            }
                        ...
                        if state.match_available != 0 {
                            let Some(literal) =
                                working.slow_literal_at(state.strstart.wrapping_sub(1))
                            else {
                                break 'slow need_more;
                            };
                        ...
                        if avail_out == 0 {
                            break 'slow if finish { finish_started } else { need_more };
                        }
                        let output = core::slice::from_raw_parts_mut(next_out, avail_out as usize);
                        let Some(result) =
                            working.flush_pending_into(state, output, avail_out, total_out)
...
// state, stream, and callback-storage crossings belong to the exported
// `deflate_ffi` boundary.  Keep this expansion at that boundary until the
// one-call `DeflateWorkingSet` owns the whole strategy.
#[macro_export]
macro_rules! deflate_stored_at_ffi_boundary {
    ($s:expr, $flush:expr) => {{
        let mut s = $s;
        let mut flush = $flush;
        'stored: {
            ...
            let mut min_block: ::core::ffi::c_uint =
                stored_block_min_size(pending_buf_size, w_size);
            ...
            loop {
                ...
                if window_len != 0 {
                    ...
                    if !stored_block_copy_buffered_output(
                        window,
                        output,
                        (*s).block_start,
                        window_len,
                    ) {
                        break 'stored need_more;
                    }
...
// As with RLE, this strategy's remaining raw state and callback-storage
// crossings stay directly at the exported `deflate_ffi` boundary until the
// full per-call working-set owner can replace them.
#[macro_export]
macro_rules! deflate_huff_at_ffi_boundary {
    ($s:expr, $flush:expr) => {{
        let s = $s;
        let flush = $flush;
        'huff: {
...
mod tests {
    ...
    fn drain_pending_copies_the_selected_pending_segment() {
        let mut pending = [0; 12];
        pending[..10].copy_from_slice(b"0123456789");
        let storage = PendingStorageView::new(&mut pending, pending_storage_layout(3)).unwrap();
    ...
    fn drain_pending_rejects_invalid_storage_or_output_without_writing() {
        let mut pending = [0; 8];
        pending[..6].copy_from_slice(b"012345");
