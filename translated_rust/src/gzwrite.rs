...
/// migrated, but callers only need the buffered/direct compression operation.
/// Keeping that operation behind this facade avoids passing an unsafe closure
/// through the zero-fill state machine.
struct GzCompressor<'a> {
...
impl GzCompressor<'_> {
    ...
    fn buffered(&mut self, flush: ::core::ffi::c_int) -> ::core::ffi::c_int {
        self.compress(flush, Some(GzCompInput::Buffered))
    }
}
...
fn gz_zero_impl(compressor: &mut GzCompressor<'_>) -> ::core::ffi::c_int {
    if compressor.state.strm.avail_in != 0
        && compressor.buffered(crate::zlib_h::Z_NO_FLUSH) == -1 as ::core::ffi::c_int
    {
        return -1 as ::core::ffi::c_int;
    }
    let mut first = true;
    loop {
        ...
        let ret = compressor.buffered(crate::zlib_h::Z_NO_FLUSH);
        let state = &mut *compressor.state;
        let n = n.wrapping_sub(state.strm.avail_in as ::core::ffi::c_uint);
