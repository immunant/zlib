...
pub use crate::src::gzlib::gz_error_safe as gz_error;
...
fn read_avail(state: &mut crate::gzguts_h::gz_state) -> ::core::ffi::c_int {
    ...
    if available > state.in_end
        || state.in_end > state.in_buf.len()
        || available > state.size as usize
    {
...
fn read_decomp(state: &mut crate::gzguts_h::gz_state, output: &mut [u8]) -> ::core::ffi::c_int {
    ...
    loop {
        ...
        let inflater = state
            .gzip_inflater
            .as_mut()
            .expect("gzip inflater must be initialized before decompression");
        let before_in = inflater.total_in();
        let before_out = inflater.total_out();
        let status = inflater.decompress(
            &state.in_buf[start..state.in_end],
            &mut output[written..],
        ...
        state.strm.avail_in = state
            .strm
            .avail_in
            .saturating_sub(consumed as crate::stdlib::uInt);
        state.strm.total_in = state
            .strm
            .total_in
            .wrapping_add(consumed as crate::stdlib::uLong);
        state.strm.total_out = state
            .strm
            .total_out
            .wrapping_add(produced as crate::stdlib::uLong);
        written += produced;
        match status {
            Ok(Status::StreamEnd) => {
                state.junk = 0;
        ...
        match state.how {
            crate::gzguts_h::LOOK => {
                if read_look(state) == -1 || state.how == crate::gzguts_h::LOOK {
                    return if state.err == crate::zlib_h::Z_OK
            ...
            let end = match state.x.next.checked_add(n as usize) {
                ...
                _ => {
                    crate::src::gzlib::gz_error_safe(
                        state,
                        crate::zlib_h::Z_STREAM_ERROR,
                        Some(c"output buffer state corrupt"),
                    );
                    return -1;
                }
            };
...
pub fn gzfread(
    ...
    if len > buf.len() {
        crate::src::gzlib::gz_error_safe(
            state,
            crate::zlib_h::Z_STREAM_ERROR,
            Some(c"output buffer too small"),
        );
        return 0;
    }
...
pub fn gzgetc(state: &mut crate::gzguts_h::gz_state) -> ::core::ffi::c_int {
    ...
    let mut buf = [0u8; 1];
    if read_gz_read(state, &mut buf) < 1 {
        -1
    } else {
...
pub fn gzungetc(
    ...
    let capacity = match (state.size as usize).checked_mul(2) {
        Some(capacity) if capacity == state.out_buf.len() => capacity,
        _ => {
            crate::src::gzlib::gz_error_safe(
                state,
                crate::zlib_h::Z_STREAM_ERROR,
    ...
    state.out_buf[state.x.next] = c as u8;
    state.x.pos -= 1;
    state.past = 0;
    c
}
...
pub unsafe extern "C" fn gzungetc_ffi(
    ...
    if file.is_null() {
        ...
        gzungetc(&mut *(file as crate::gzguts_h::gz_statep), c)
    }
}
pub fn gzgets(state: &mut crate::gzguts_h::gz_state, buf: &mut [u8]) -> bool {
    if state.mode != crate::gzguts_h::GZ_READ || buf.is_empty() {
        return false;
    }
    if (state.err != crate::zlib_h::Z_OK && state.err != crate::zlib_h::Z_BUF_ERROR)
        && state.again == 0
    ...
    while written + 1 < buf.len() {
        if state.x.have == 0 && read_fetch(state) == -1 {
            break;
        }
        if state.x.have == 0 {
            state.past = 1;
        ...
        let available = state.x.have as usize;
        let end = match state.x.next.checked_add(available) {
            Some(end) if end <= state.out_buf.len() => end,
            _ => {
                crate::src::gzlib::gz_error_safe(
                    state,
        ...
        let input = &state.out_buf[state.x.next..end];
        let count = input.len().min(buf.len() - written - 1);
        let count = input
            .iter()
            .position(|&byte| byte == b'\n')
            .map_or(count, |at| at + 1);
        buf[written..written + count].copy_from_slice(&input[..count]);
...
pub unsafe extern "C" fn gzgets_ffi(
    ...
    let output = ::core::slice::from_raw_parts_mut(buf as *mut u8, len as usize);
    if gzgets(&mut *(file as crate::gzguts_h::gz_statep), output) {
        buf
    } else {
        ::core::ptr::null_mut()
    }
}
pub fn gzdirect(state: &mut crate::gzguts_h::gz_state) -> ::core::ffi::c_int {
    if state.mode == crate::gzguts_h::GZ_READ
        && state.how == crate::gzguts_h::LOOK
        && state.x.have == 0
    {
