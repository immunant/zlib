...
fn send_bits_state(
    ...
    if bi_valid > crate::src::deflate::Buf_size - bit_len {
        ...
        SendBitsResult {
            ...
            len: 2,
            bi_buf: (val >> (crate::src::deflate::Buf_size - bi_valid)) as crate::zutil_h::ush,
            bi_valid: bi_valid + bit_len - crate::src::deflate::Buf_size,
        }
    } else {
        SendBitsResult {
            bytes: [0, 0],
...
fn scan_tree(
    ...
    while n <= max_code {
        curlen = nextlen;
        nextlen = tree[(n + 1 as ::core::ffi::c_int) as usize].dad as ::core::ffi::c_int;
        count += 1;
        if !(count < max_count && curlen == nextlen) {
            if count < min_count {
                bl_tree[curlen as usize].freq = (bl_tree[curlen as usize].freq
                    as ::core::ffi::c_int
...
impl PendingBitSink<'_> {
    fn append_bytes(&mut self, bytes: &[crate::stdlib::Byte]) {
        append_pending_bytes(self.pending_buf, self.pending, bytes);
    }

    fn send_bits(&mut self, value: ::core::ffi::c_int, bit_len: ::core::ffi::c_int) {
        let bits = send_bits_state(*self.bi_buf, *self.bi_valid, value, bit_len);
        self.append_bytes(&bits.bytes[..bits.len]);
        *self.bi_buf = bits.bi_buf;
        *self.bi_valid = bits.bi_valid;
    }

    fn send_code(&mut self, tree: &[crate::src::deflate::ct_data], code: usize) {
        self.send_bits(
            tree[code].freq as ::core::ffi::c_int,
            tree[code].dad as ::core::ffi::c_int,
        );
    }
}
    ...
    while n <= max_code {
        ...
        if !(count < max_count && curlen == nextlen) {
            if count < min_count {
                ...
                sink.send_code(bl_tree, 16 as usize);
                sink.send_bits(count - 3 as ::core::ffi::c_int, 2 as ::core::ffi::c_int);
            } else if count <= 10 as ::core::ffi::c_int {
                sink.send_code(bl_tree, 17 as usize);
                sink.send_bits(count - 3 as ::core::ffi::c_int, 3 as ::core::ffi::c_int);
            } else {
                sink.send_code(bl_tree, 18 as usize);
                sink.send_bits(count - 11 as ::core::ffi::c_int, 7 as ::core::ffi::c_int);
            }
            count = 0 as ::core::ffi::c_int;
            prevlen = curlen;
            (max_count, min_count) = tree_run_limits(Some(curlen), nextlen);
...
fn send_all_trees(
    ...
    sink.append_bytes(&header_bits.bytes[..header_bits.len]);
    *sink.bi_buf = header_bits.bi_buf;
    *sink.bi_valid = header_bits.bi_valid;
    send_tree(
        &state.dyn_ltree,
...
pub(crate) fn stored_block_len_bytes(stored_len: crate::zutil_h::ulg) -> [crate::stdlib::Bytef; 4] {
    let len = stored_len as crate::zutil_h::ush;
    let nlen = !len;
    [
        (len as ::core::ffi::c_int & 0xff as ::core::ffi::c_int) as crate::stdlib::Bytef,
        (len as ::core::ffi::c_int >> 8 as ::core::ffi::c_int) as crate::stdlib::Bytef,
        (nlen as ::core::ffi::c_int & 0xff as ::core::ffi::c_int) as crate::stdlib::Bytef,
...
fn compress_block(
    ...
    while sx < sym_next as usize {
        ...
        if dist == 0 as ::core::ffi::c_uint {
            ...
            let mut code = crate::src::trees::_length_code[lc as usize] as ::core::ffi::c_uint;
            sink.send_code(
                ltree,
                code.wrapping_add(256 as ::core::ffi::c_uint)
                    .wrapping_add(1 as ::core::ffi::c_uint) as usize,
            );
            let mut extra = extra_lbits[code as usize];
            if extra != 0 as ::core::ffi::c_int {
                lc -= base_length[code as usize];
                sink.send_bits(lc, extra);
            }

            dist = dist.wrapping_sub(1);
            code =
                crate::src::trees::_dist_code[dist_code_table_index(dist)] as ::core::ffi::c_uint;
            sink.send_code(dtree, code as usize);
            extra = extra_dbits[code as usize];
            if extra != 0 as ::core::ffi::c_int {
                dist = dist.wrapping_sub(base_dist[code as usize] as ::core::ffi::c_uint);
                sink.send_bits(dist as ::core::ffi::c_int, extra);
            }
        }
    }
    sink.send_code(ltree, END_BLOCK as usize);
}
