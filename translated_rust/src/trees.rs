...
pub mod trees_h {
    ...
    /// Immutable fixed-Huffman entry.  Unlike dynamic `ct_data`, this table
    /// is never repurposed from frequencies to codes, so it need not model
    /// those phase-dependent values as unions.
    #[derive(Copy, Clone)]
    #[repr(C)]
    pub struct StaticCtData {
        pub code: crate::zutil_h::ush,
        pub len: crate::zutil_h::ush,
    }

    pub static static_ltree: [StaticCtData; 288] = [
        StaticCtData {
    ...
    pub static static_dtree: [StaticCtData; 30] = [
        StaticCtData {
            code: 0 as crate::zutil_h::ush,
...
pub struct static_tree_desc_s {
    pub static_tree: Option<&'static [trees_h::StaticCtData]>,
...
fn static_desc(kind: u8) -> &'static crate::src::deflate::static_tree_desc {
    match kind {
        crate::src::deflate::STATIC_TREE_LITERAL => &static_l_desc,
        crate::src::deflate::STATIC_TREE_DISTANCE => &static_d_desc,
        crate::src::deflate::STATIC_TREE_BIT_LENGTH => &static_bl_desc,
        _ => &static_bl_desc,
    }
}
...
pub(crate) fn flush_bits_impl(s: &mut crate::src::deflate::deflate_state) {
    if s.bi_valid == 16 as ::core::ffi::c_int {
        ...
        s.put_pending_byte(s.bi_buf as crate::stdlib::Byte);
...
unsafe fn gen_bitlen(s: &mut crate::src::deflate::deflate_state, tree_kind: u8) {
        ...
        if n <= max_code {
            ...
            if let Some(stree) = stree {
                s.static_len = s
                    .static_len
                    .wrapping_add((f as crate::zutil_h::ulg).wrapping_mul(
                        (stree[n as usize].len as ::core::ffi::c_int + xbits) as ::core::ffi::c_uint
    ...
    while bits != 0 as ::core::ffi::c_int {
        ...
        while n != 0 as ::core::ffi::c_int {
            ...
            if tree[m as usize].len() as ::core::ffi::c_uint != bits as ::core::ffi::c_uint {
...
unsafe fn build_tree_impl(s: &mut crate::src::deflate::deflate_state, tree_kind: u8) {
    ...
    while s.heap_len < 2 as ::core::ffi::c_int {
        s.heap_len += 1;
        let heap_index = s.heap_len as usize;
        s.heap[heap_index] = if max_code < 2 as ::core::ffi::c_int {
            max_code += 1;
            max_code
        } else {
            0 as ::core::ffi::c_int
        ...
        selected_tree(s, tree_kind)[node as usize].fc = 1 as crate::zutil_h::ush;
    ...
    match tree_kind {
        crate::src::deflate::STATIC_TREE_LITERAL => s.l_desc.max_code = max_code,
        crate::src::deflate::STATIC_TREE_DISTANCE => s.d_desc.max_code = max_code,
        _ => s.bl_desc.max_code = max_code,
    }
    ...
    loop {
        ...
        selected_tree(s, tree_kind)[node as usize].fc =
        ...
        selected_tree(s, tree_kind)[m as usize].dl = node as crate::zutil_h::ush;
        selected_tree(s, tree_kind)[n as usize].dl = node as crate::zutil_h::ush;
...
fn scan_tree(
    ...
    let mut nextlen: ::core::ffi::c_int = tree[0].len() as ::core::ffi::c_int;
    ...
    tree[sentinel].set_len(0xffff as ::core::ffi::c_int as crate::zutil_h::ush);
    n = 0 as ::core::ffi::c_int;
    while (n as usize) <= max_code {
        curlen = nextlen;
        nextlen = tree[n as usize + 1].len() as ::core::ffi::c_int;
        count += 1;
        if !(count < max_count && curlen == nextlen) {
            if count < min_count {
                if let Some(entry) = bl_tree.get_mut(curlen as usize) {
                    entry.fc = entry.freq().wrapping_add(count as crate::zutil_h::ush);
                }
            } else if curlen != 0 as ::core::ffi::c_int {
                if curlen != prevlen {
                    if let Some(entry) = bl_tree.get_mut(curlen as usize) {
                        entry.fc = entry.freq().wrapping_add(1);
                    }
                }
                if let Some(entry) = bl_tree.get_mut(REP_3_6 as usize) {
                    entry.fc = entry.freq().wrapping_add(1);
                }
            } else if count <= 10 as ::core::ffi::c_int {
                if let Some(entry) = bl_tree.get_mut(REPZ_3_10 as usize) {
                    entry.fc = entry.freq().wrapping_add(1);
                }
            } else {
                if let Some(entry) = bl_tree.get_mut(REPZ_11_138 as usize) {
                    entry.fc = entry.freq().wrapping_add(1);
                }
            }
...
fn send_bits(
    ...
    if s.bi_valid > crate::src::deflate::Buf_size - len {
        s.bi_buf = (s.bi_buf as ::core::ffi::c_int
            | (value as crate::zutil_h::ush as ::core::ffi::c_int) << s.bi_valid)
            as crate::zutil_h::ush;
        s.put_pending_byte((s.bi_buf as ::core::ffi::c_int & 0xff) as crate::zutil_h::uch);
        s.put_pending_byte((s.bi_buf as ::core::ffi::c_int >> 8) as crate::zutil_h::uch);
        s.bi_buf = (value as crate::zutil_h::ush as ::core::ffi::c_int
...
unsafe fn send_tree(
    ...
    while n <= max_code {
        curlen = nextlen;
        nextlen = tree[(n + 1 as ::core::ffi::c_int) as usize].len() as ::core::ffi::c_int;
        count += 1;
        if !(count < max_count && curlen == nextlen) {
            if count < min_count {
                loop {
                    send_code(s, curlen as usize);
                    count -= 1;
                ...
                send_code(s, REP_3_6 as usize);
                send_bits(s, count - 3 as ::core::ffi::c_int, 2 as ::core::ffi::c_int);
            } else if count <= 10 as ::core::ffi::c_int {
                send_code(s, REPZ_3_10 as usize);
                send_bits(s, count - 3 as ::core::ffi::c_int, 3 as ::core::ffi::c_int);
            } else {
                send_code(s, REPZ_11_138 as usize);
                send_bits(s, count - 11 as ::core::ffi::c_int, 7 as ::core::ffi::c_int);
            }
            count = 0 as ::core::ffi::c_int;
            prevlen = curlen;
            if nextlen == 0 as ::core::ffi::c_int {
...
unsafe fn send_all_trees(
    ...
    while rank < blcodes {
        send_bits(
            s,
            s.bl_tree[bl_order[rank as usize] as usize].len() as ::core::ffi::c_int,
            3,
        );
        rank += 1;
    }
    let ltree = s.dyn_ltree;
    send_tree(s, &ltree, lcodes - 1);
...
pub unsafe extern "C" fn _tr_stored_block(
    ...
    let buf = if stored_len == 0 {
        ...
        let Ok(len) = usize::try_from(stored_len) else {
            return;
        };
        if buf.is_null() {
            return;
...
unsafe fn tr_align(s: &mut crate::src::deflate::deflate_state) {
    ...
    let len_0: ::core::ffi::c_int = static_ltree[256].len as ::core::ffi::c_int;
    if s.bi_valid > crate::src::deflate::Buf_size - len_0 {
        let val_0: ::core::ffi::c_int = static_ltree[256].code as ::core::ffi::c_int;
        s.bi_buf = (s.bi_buf as ::core::ffi::c_int
        ...
        s.bi_buf = (s.bi_buf as ::core::ffi::c_int
            | (static_ltree[256].code as ::core::ffi::c_int) << s.bi_valid)
            as crate::zutil_h::ush;
        s.bi_valid += len_0;
    }
    flush_bits_impl(s);
...
unsafe extern "C" fn compress_block(
    ...
    if (*s).bi_valid > crate::src::deflate::Buf_size - len_4 {
        let mut val_4: ::core::ffi::c_int =
            (*ltree.offset(256 as isize)).fc as ::core::ffi::c_int;
        (*s).bi_buf = ((*s).bi_buf as ::core::ffi::c_int
    ...
    if stored_len.wrapping_add(4 as crate::zutil_h::ulg) <= opt_lenb && buf.is_some() {
        ...
        if s.bi_valid > crate::src::deflate::Buf_size - len {
            ...
            s.bi_buf = (s.bi_buf as ::core::ffi::c_int
                | (val as crate::zutil_h::ush as ::core::ffi::c_int) << s.bi_valid)
                as crate::zutil_h::ush;
            s.put_pending_byte((s.bi_buf as ::core::ffi::c_int
...
pub unsafe fn _tr_tally(
    ...
    if dist == 0 as ::core::ffi::c_uint {
        ...
        tree.fc = tree.fc.wrapping_add(1);
    } else {
