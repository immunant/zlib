...
fn bi_flush_state(
    ...
    if *bi_valid == 16 as ::core::ffi::c_int {
        let bytes = [
            *bi_buf as crate::stdlib::Byte,
            (*bi_buf >> 8) as crate::stdlib::Byte,
        ];
        if append_pending(pending_buf, pending, &bytes) {
            *bi_buf = 0 as crate::zutil_h::ush;
            *bi_valid = 0 as ::core::ffi::c_int;
        }
    } else if *bi_valid >= 8 as ::core::ffi::c_int
        && append_pending(pending_buf, pending, &[*bi_buf as crate::stdlib::Byte])
    {
        *bi_buf = (*bi_buf >> 8) as crate::zutil_h::ush;
...
fn bi_windup_state(
    ...
    let bytes = if *bi_valid > 8 as ::core::ffi::c_int {
        [
            *bi_buf as crate::stdlib::Byte,
            (*bi_buf >> 8) as crate::stdlib::Byte,
        ]
    } else if *bi_valid > 0 as ::core::ffi::c_int {
        [*bi_buf as crate::stdlib::Byte, 0]
    } else {
        [0, 0]
    };
    let count = if *bi_valid > 8 as ::core::ffi::c_int {
        2
    } else if *bi_valid > 0 as ::core::ffi::c_int {
        1
    } else {
    ...
    if count != 0 && !append_pending(pending_buf, pending, &bytes[..count]) {
        return false;
    }
    *bi_used =
...
fn send_bits_state(
    ...
    if !(0..=crate::src::deflate::Buf_size).contains(&len)
        || !(0..=crate::src::deflate::Buf_size).contains(bi_valid)
    {
        return false;
    }
    if *bi_valid > crate::src::deflate::Buf_size - len {
        let combined = *bi_buf as ::core::ffi::c_int
            | ((value as crate::zutil_h::ush as ::core::ffi::c_int) << *bi_valid);
        let bytes = [
            combined as crate::stdlib::Byte,
            (combined >> 8) as crate::stdlib::Byte,
        ];
        if !append_pending(pending_buf, pending, &bytes) {
            return false;
        }
        *bi_buf = (value as crate::zutil_h::ush as ::core::ffi::c_int
...
pub(crate) fn tr_stored_block_state(
    ...
    if !send_bits_state(pending_buf, pending, bi_buf, bi_valid, last, 3) {
        return false;
    }
    if !bi_windup_state(pending_buf, pending, bi_buf, bi_valid, bi_used) {
        return false;
    }
    let stored_len = stored.len() as crate::zutil_h::ulg;
    let header = [
        stored_len as crate::zutil_h::ush as crate::stdlib::Byte,
        (stored_len as crate::zutil_h::ush >> 8) as crate::stdlib::Byte,
        (!stored_len as crate::zutil_h::ush) as crate::stdlib::Byte,
        (!stored_len as crate::zutil_h::ush >> 8) as crate::stdlib::Byte,
    ];
...
pub(crate) fn tr_align_state(
    ...
    if !send_bits_state(pending_buf, pending, bi_buf, bi_valid, 2, 3) {
        return;
    }
    if send_bits_state(
        pending_buf,
        ...
        bi_buf,
        bi_valid,
        static_ltree[256].fc.code as ::core::ffi::c_int,
        static_ltree[256].dl.len as ::core::ffi::c_int,
    ) {
        bi_flush_state(pending_buf, pending, bi_buf, bi_valid);
    }
}
...
fn gen_bitlen_state(
    ...
    bl_count: &mut [crate::zutil_h::ush; crate::src::deflate::MAX_BITS as usize + 1],
    opt_len: &mut crate::zutil_h::ulg,
    static_len: &mut crate::zutil_h::ulg,
) -> bool {
    let Ok(heap_max) = usize::try_from(heap_max) else {
        return false;
    };
    let max_length = stat_desc.max_length;
    if !(1..=crate::src::deflate::MAX_BITS).contains(&max_length) {
        return false;
    }
    bl_count.fill(0);
    let Some(&root) = heap.get(heap_max) else {
        return false;
    };
    let Ok(root) = usize::try_from(root) else {
        return false;
    };
    let Some(root) = tree.get_mut(root) else {
        return false;
    };
    root.dl.len = 0;
    ...
    while h < crate::src::deflate::HEAP_SIZE as usize {
        let Some(&node) = heap.get(h) else {
            return false;
        };
        let Ok(node) = usize::try_from(node) else {
            return false;
        };
        let Some(parent) = tree.get(node).map(|entry| entry.dl.dad as usize) else {
            return false;
        };
        let Some(parent_len) = tree.get(parent).map(|entry| entry.dl.len) else {
            return false;
        };
        let mut bits = parent_len as ::core::ffi::c_int + 1;
        if bits > max_length {
            bits = max_length;
            overflow += 1;
        }
        let Some(entry) = tree.get_mut(node) else {
            return false;
        };
        entry.dl.len = bits as crate::zutil_h::ush;
        if node as ::core::ffi::c_int <= max_code {
            let Some(count) = bl_count.get_mut(bits as usize) else {
                return false;
            };
            *count = count.wrapping_add(1);
            let xbits = if node as ::core::ffi::c_int >= stat_desc.extra_base {
                let offset = node as ::core::ffi::c_int - stat_desc.extra_base;
                let Ok(offset) = usize::try_from(offset) else {
                    return false;
                };
                let Some(&bits) = stat_desc.extra_bits.get(offset) else {
                    return false;
                };
            ...
            *opt_len = opt_len.wrapping_add(
                (entry.fc.freq as crate::zutil_h::ulg)
                    .wrapping_mul((bits + xbits) as crate::zutil_h::ulg),
            );
            if let Some(stree) = stat_desc.static_tree {
                let Some(static_entry) = stree.get(node) else {
                    return false;
                };
                *static_len =
                    static_len.wrapping_add((entry.fc.freq as crate::zutil_h::ulg).wrapping_mul(
                        (static_entry.dl.len as ::core::ffi::c_int + xbits) as crate::zutil_h::ulg,
                    ));
            }
        }
        h += 1;
    ...
    while overflow > 0 {
        ...
        bl_count[(bits + 1) as usize] = bl_count[(bits + 1) as usize].wrapping_add(2);
        bl_count[max_length as usize] = bl_count[max_length as usize].wrapping_sub(1);
        overflow -= 2;
    }
    let mut bits = max_length;
    while bits != 0 {
        let mut count = bl_count[bits as usize];
        while count != 0 {
            h = match h.checked_sub(1) {
                Some(value) => value,
                None => return false,
            };
            let Some(&node) = heap.get(h) else {
            ...
            if node as ::core::ffi::c_int <= max_code {
                ...
                if entry.dl.len as ::core::ffi::c_int != bits {
                    *opt_len = opt_len.wrapping_add(
                        (bits as crate::zutil_h::ulg)
                            .wrapping_sub(entry.dl.len as crate::zutil_h::ulg)
                            .wrapping_mul(entry.fc.freq as crate::zutil_h::ulg),
                    );
                    entry.dl.len = bits as crate::zutil_h::ush;
                }
            }
...
fn build_tree_state(
    ...
    let elems = usize::try_from(stat_desc.elems).ok()?;
    let tree_len = elems.checked_mul(2)?.checked_add(1)?;
    if elems < 2 || tree.len() < tree_len || tree_len > heap.len() || tree_len > depth.len() {
        return None;
    }
    ...
    *heap_max = crate::src::deflate::HEAP_SIZE;
    let mut max_code = -1;
    for (n, entry) in tree[..elems].iter_mut().enumerate() {
        if entry.fc.freq != 0 {
            max_code = n as ::core::ffi::c_int;
            *heap_len += 1;
    ...
    while *heap_len < 2 {
        ...
        *heap.get_mut(*heap_len as usize)? = node as ::core::ffi::c_int;
        tree.get_mut(node)?.fc.freq = 1;
        *depth.get_mut(node)? = 0;
        *opt_len = opt_len.wrapping_sub(1);
        if let Some(stree) = stat_desc.static_tree {
            *static_len = static_len.wrapping_sub(stree.get(node)?.dl.len as crate::zutil_h::ulg);
        }
    }

    let mut n = *heap_len / 2;
    while n >= 1 {
        if !pqdownheap_state(heap, *heap_len, depth, tree, n) {
            return None;
        }
        n -= 1;
    }
    let mut node = elems;
    loop {
        let n = heap[SMALLEST as usize];
        let prior_heap_len = *heap_len;
        *heap_len -= 1;
        heap[SMALLEST as usize] = *heap.get(prior_heap_len as usize)?;
        if !pqdownheap_state(heap, *heap_len, depth, tree, SMALLEST) {
            return None;
        }
        let m = heap[SMALLEST as usize];
        *heap_max -= 1;
        *heap.get_mut(*heap_max as usize)? = n;
        *heap_max -= 1;
        *heap.get_mut(*heap_max as usize)? = m;
        let n = usize::try_from(n).ok()?;
        let m = usize::try_from(m).ok()?;
        let frequency = tree.get(n)?.fc.freq.wrapping_add(tree.get(m)?.fc.freq);
        tree.get_mut(node)?.fc.freq = frequency;
        depth[node] = depth[n].max(depth[m]).wrapping_add(1);
        tree.get_mut(m)?.dl.dad = node as crate::zutil_h::ush;
        tree.get_mut(n)?.dl.dad = node as crate::zutil_h::ush;
        heap[SMALLEST as usize] = node as ::core::ffi::c_int;
        node += 1;
        if !pqdownheap_state(heap, *heap_len, depth, tree, SMALLEST) {
            return None;
        }
    ...
    *heap_max -= 1;
    ...
    if !gen_bitlen_state(
        tree, max_code, stat_desc, heap, *heap_max, bl_count, opt_len, static_len,
    ) || !gen_codes_state(tree, max_code, bl_count)
    {
        return None;
    }
    Some(max_code)
...
fn scan_tree_state(
    ...
    if sentinel >= tree.len() || bl_tree.len() < crate::src::deflate::BL_CODES as usize {
        return false;
    }
    let mut n = 0usize;
    ...
    tree[sentinel].dl.len = 0xffff as ::core::ffi::c_int as crate::zutil_h::ush;
    while n <= max_code {
        curlen = nextlen;
        nextlen = tree[n + 1].dl.len as ::core::ffi::c_int;
        count += 1;
        if !(count < max_count && curlen == nextlen) {
            let Ok(code) = usize::try_from(curlen) else {
                return false;
            };
            if code >= bl_tree.len() {
                return false;
            ...
            if count < min_count {
                bl_tree[code].fc.freq = (bl_tree[code].fc.freq as ::core::ffi::c_int
                    + count as crate::zutil_h::ush as ::core::ffi::c_int)
                    as crate::zutil_h::ush;
            } else if curlen != 0 as ::core::ffi::c_int {
                if curlen != prevlen {
                    bl_tree[code].fc.freq = bl_tree[code].fc.freq.wrapping_add(1);
                }
...
fn build_bl_tree_state(s: &mut crate::src::deflate::deflate_state) -> Option<::core::ffi::c_int> {
    if !scan_tree_state(&mut s.bl_tree, &mut s.dyn_ltree, s.l_desc.max_code)
        || !scan_tree_state(&mut s.bl_tree, &mut s.dyn_dtree, s.d_desc.max_code)
    {
        return None;
    }
    let stat_desc = s.bl_desc.stat_desc?;
    let max_code = build_tree_state(
        &mut s.bl_tree,
        stat_desc,
        &mut s.heap,
        &mut s.heap_len,
...
fn send_all_trees_header_state(
    ...
    if blcodes > crate::src::deflate::BL_CODES {
        return false;
    }
    if !send_bits_state(
        pending_buf,
    ...
    let mut rank = 0;
    while rank < blcodes {
        let Some(&code) = bl_order.get(rank as usize) else {
            return false;
        };
        let Some(node) = bl_tree.get(code as usize) else {
            return false;
        };
        if !send_bits_state(
            pending_buf,
...
pub unsafe extern "C" fn _tr_stored_block_ffi(
    ...
    if (pending_len != 0 && s.pending_buf.is_none()) || (stored_len != 0 && buf.is_null()) {
        return;
    }
    ...
    let stored = if stored_len == 0 {
        ...
        ::core::slice::from_raw_parts(buf as *const crate::stdlib::Byte, stored_len)
    };
...
pub unsafe extern "C" fn _tr_flush_bits_ffi(mut s: *mut crate::src::deflate::deflate_state) {
    if s.is_null() || (*s).pending_buf.is_none() {
        return;
    }
    let Ok(pending_len) = usize::try_from((*s).pending_buf_size) else {
        return;
...
fn compress_block_state(
    ...
    bi_buf: &mut crate::zutil_h::ush,
    bi_valid: &mut ::core::ffi::c_int,
    symbols: &[crate::zutil_h::uch],
    ltree: &[crate::src::deflate::ct_data],
    dtree: &[crate::src::deflate::ct_data],
) -> bool {
    let mut symbols = symbols.chunks_exact(3);
    if !symbols.remainder().is_empty() {
        return false;
    }
    for symbol in &mut symbols {
        let mut dist = (symbol[0] as ::core::ffi::c_uint)
            .wrapping_add((symbol[1] as ::core::ffi::c_uint) << 8);
        let mut lc = symbol[2] as ::core::ffi::c_int;
        if dist == 0 {
            if !send_tree_code_state(pending_buf, pending, bi_buf, bi_valid, ltree, lc, 0, 0) {
                return false;
            }
            continue;
        }

        let Some(&length_code) = crate::src::trees::_length_code.get(lc as usize) else {
            return false;
        };
        let code = length_code as usize;
        let Some(&extra) = extra_lbits.get(code) else {
            return false;
        };
        if !send_tree_code_state(
            pending_buf,
        ...
        if extra != 0 {
            let Some(&base) = base_length.get(code) else {
                return false;
            };
            lc -= base;
            if !send_bits_state(pending_buf, pending, bi_buf, bi_valid, lc, extra) {
                return false;
            }
        }

        dist = dist.wrapping_sub(1);
        let dist_index = if dist < 256 {
            dist as usize
        } else {
            256usize.wrapping_add((dist >> 7) as usize)
        };
        let Some(&distance_code) = crate::src::trees::_dist_code.get(dist_index) else {
            return false;
        };
        let code = distance_code as usize;
        let Some(&extra) = extra_dbits.get(code) else {
            return false;
        };
        if !send_tree_code_state(
            pending_buf,
        ...
        if extra != 0 {
            let Some(&base) = base_dist.get(code) else {
                return false;
            };
            dist = dist.wrapping_sub(base as ::core::ffi::c_uint);
            if !send_bits_state(
                pending_buf,
                pending,
                bi_buf,
...
fn detect_data_type(tree: &[crate::src::deflate::ct_data]) -> ::core::ffi::c_int {
    ...
    while n <= 31 as ::core::ffi::c_int {
        if block_mask & 1 as ::core::ffi::c_ulong != 0
            && tree[n as usize].fc.freq as ::core::ffi::c_int != 0 as ::core::ffi::c_int
        {
    ...
    if tree[9 as ::core::ffi::c_int as usize].fc.freq as ::core::ffi::c_int
        != 0 as ::core::ffi::c_int
        || tree[10 as ::core::ffi::c_int as usize].fc.freq as ::core::ffi::c_int
            != 0 as ::core::ffi::c_int
        || tree[13 as ::core::ffi::c_int as usize].fc.freq as ::core::ffi::c_int
            != 0 as ::core::ffi::c_int
    {
...
pub fn _tr_tally(
    ...
    let Ok(start) = usize::try_from(*sym_next) else {
        return 0;
    };
    let Some(end) = start.checked_add(3) else {
        return 0;
    };
    if end > symbols.len() || end > sym_end as usize {
        return 0;
    }
    let Some(symbol) = symbols.get_mut(start..end) else {
        return 0;
    };
    symbol[0] = dist as crate::zutil_h::uch;
    symbol[1] = (dist >> 8) as crate::zutil_h::uch;
    symbol[2] = lc as crate::zutil_h::uch;
    *sym_next = end as crate::stdlib::uInt;

    if dist == 0 {
        let Some(node) = dyn_ltree.get_mut(lc as usize) else {
            return 0;
        };
        node.fc.freq = node.fc.freq.wrapping_add(1);
    } else {
        *matches = matches.wrapping_add(1);
        ...
        let Some(node) = dyn_ltree.get_mut(length_code as usize + 257) else {
            return 0;
        };
        node.fc.freq = node.fc.freq.wrapping_add(1);
        ...
        let Some(node) = dyn_dtree.get_mut(distance_code as usize) else {
            return 0;
        };
        node.fc.freq = node.fc.freq.wrapping_add(1);
    }
...
pub unsafe extern "C" fn _tr_tally_ffi(
    ...
    if symbol_end > pending_len || (pending_len != 0 && state.pending_buf.is_none()) {
        return 0;
    }
    let pending_buf = if pending_len == 0 {
        &mut []
