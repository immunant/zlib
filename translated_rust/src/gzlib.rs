...
unsafe fn gz_open(
    ...
    state_ptr.as_ptr().write(crate::gzguts_h::gz_state {
        ...
        in_0: ::core::mem::ManuallyDrop::new(Vec::new()),
        out: ::core::mem::ManuallyDrop::new(Vec::new()),
        direct: 0,
        junk: 0,
        how: 0,
        again: 0,
    ...
    for &mode in mode.to_bytes() {
            ...
            && mode as ::core::ffi::c_int <= '9' as ::core::ffi::c_int
        {
            ...
            match mode as ::core::ffi::c_int {
                ...
                43 => {
                    crate::stdlib::free(state_ptr.as_ptr() as *mut ::core::ffi::c_void);
                    return None;
                }
                101 => {
...
fn gzdopen(
    ...
    // `gz_open` initializes and owns the state allocation before returning its handle.
    unsafe { gz_open(&path, fd, mode) }
}
#[export_name = "gzdopen"]
...
pub fn gzseek64(
    ...
    {
        match rustix::fs::seek(
            fd,
            rustix::fs::SeekFrom::Current(offset - state.x.have as crate::stdlib::off64_t),
        ) {
            Ok(_) => {}
            Err(_) => return -1 as crate::stdlib::off64_t,
        }
        state.x.have = 0 as ::core::ffi::c_uint;
        state.eof = 0 as ::core::ffi::c_int;
        state.past = 0 as ::core::ffi::c_int;
...
fn gzoffset64(
    ...
    let mut offset = match rustix::fs::tell(fd) {
