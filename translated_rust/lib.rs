...
pub mod gzguts_h {
    ...
    /// Storage owned by an opaque gzip handle.  ABI cursors are derived from
    /// these allocations only for the duration of translated stream calls;
    /// the allocations themselves are never manually freed.
